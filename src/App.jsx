...
const LSFLearning = () => {
  ...
  const preloadNextImages = (letter) => {
    const nextLetter =
      alphabet[(alphabet.indexOf(letter) + 1) % alphabet.length];
    const images = [`${nextLetter}.png`, `${nextLetter}_bis.png`];
  ...
  return (
    <div
      className={`flex flex-col items-center justify-center h-screen p-4 touch-manipulation transition-colors duration-200 ${darkMode ? "bg-gray-900" : "bg-gray-100"}`}
    >
      <h1
        className={`text-3xl sm:text-5xl font-bold mb-4 ${darkMode ? "text-white" : "text-gray-900"}`}
      >
        LSF Letter Learning
      </h1>
      <Card
        className={`w-full h-[80vh] ${darkMode ? "bg-gray-800 border-gray-700" : "bg-white"}`}
      >
        <CardContent className="p-4 sm:p-8 h-full">
          <div className="text-center h-full flex flex-col justify-center">
            ...
            >
              <div
                className={`text-6xl sm:text-8xl font-bold ${darkMode ? "text-white" : "text-gray-900"}`}
              >
                {currentLetter}
              </div>
            </div>
                ...
                {[currentLetter, `${currentLetter}_bis`].map((src, index) => (
                  <div key={src} className="relative">
                    ...
                    <img
                      ...
                      className={`rounded-lg shadow-lg w-[30vh] sm:w-[35vh] h-[30vh] sm:h-[35vh] object-cover transition-all duration-300 ${!showAnswer ? "blur-3xl opacity-50" : ""}`}
                      onLoad={() => {
